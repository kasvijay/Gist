...
struct RegenerateConfirmModal: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Group {
                        Text("This replaces the current summary for ") +
                        Text(sessionName).fontWeight(.medium) +
                        Text(". The transcript stays untouched.")
                    }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        // Current model display / picker toggle
                            ...
                            Button { showModelPicker.toggle() } label: {
                                HStack(spacing: 12) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        ...
                                        if let cost = info.model.costDescription {
                                            Text(cost)
                                ...
                                .padding(.vertical, 12)
                                .background(
                                    RoundedRectangle(cornerRadius: showModelPicker ? 10 : 10)
                        ...
                        HStack(alignment: .top, spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                if let info = selectedInfo {
                                    Text("Apply \(info.model.displayName) to every new summary, not just this one.")
                                        .font(.system(size: 12))
                                        .foregroundStyle(.secondary)
                                }
                            }
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ForEach(filteredModels, id: \.model.id) { item in
                        let isSelected = item.model.id == selectedModelID
                        Button {
                            ...
                            HStack(spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 1) {
                                    ...
                                    if let cost = item.model.costDescription {
                                        Text(cost)
    ...
    private var filteredModels: [ModelItem] {
        let allProviders = ProviderCatalog.summarizationProviders
        var items: [ModelItem] = []
        for p in allProviders {
            if p.requiresAPIKey && !ProviderRegistry.shared.isConnected(p.id) { continue }
            for m in p.models {
