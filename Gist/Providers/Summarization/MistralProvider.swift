...
final class MistralProvider: SummarizationProvider, Sendable {
    ...
    func summarize(
        ...
        guard let apiKey = KeychainService.shared.getKey(for: "mistral-api-key") else {
            throw ProviderError.notConfigured(.mistral)
        }

        // Mistral uses OpenAI-compatible chat completions API
