...
final class GroqSummarizationProvider: SummarizationProvider, Sendable {
    let providerID: ProviderID = .groqSummarization
    ...
    func summarize(
        ...
        guard let apiKey = KeychainService.shared.getKey(for: "groq-api-key") else {
            throw ProviderError.notConfigured(.groqSummarization)
        }
